...
namespace GDEngine {
	Teapot::Teapot(std::string name) : MeshObject(name, L"assets/meshes/txtpot.obj.txt")
	{
		this->texture = TextureLibrary::getInstance()->getTexture(TextureName::DEFAULT);
	}

	Teapot::Teapot(std::string guid, std::string name) : MeshObject(guid, name, L"assets/meshes/txtpot.obj.txt")
	{
		this->texture = TextureLibrary::getInstance()->getTexture(TextureName::DEFAULT);
	}
